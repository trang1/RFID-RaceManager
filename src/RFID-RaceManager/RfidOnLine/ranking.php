    ...
    <div class="container mt-4">
    	...
    	<div class="navigations sticky-top">
    		<ul class="nav nav-pills mb-4">
    			<li class="nav-item">
					<a class="nav-link <?= ($round=='best') ? 'active' : '' ?>" href="?q=best">
						Classificação Melhor Volta
					</a>
				</li>
				<li class="nav-item">
			...
			<table class="table table-bordered table-sm table-hover">
				...
				<tbody>
					<?php
				    	$db = new MyDB();
				    	$i=1;
				    	$result = $db->query("SELECT PilotId, PilotName, Min(BestLapTime) as BestLap FROM LapsInfos L Group By L.PilotId Order by Min(BestLapTime)");
				    	while($row = $result->fetchArray(SQLITE3_ASSOC) ){
					?>
						<tr>
							<td><?= $i ?></td>
							<td><strong><?= $row["PilotName"] ?></strong></td>
							<td><?= $row["BestLap"] ?></td>
						</tr>
					<?php
						$i++;
